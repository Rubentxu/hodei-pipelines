...
import { CreatePipelineRequest } from '@/services/pipelineApi';
...
export function PipelinesPage() {
  ...
  const { pipelines, isLoading, isError, error, createPipeline, deletePipeline, executePipeline } = usePipelines();
  ...
  const handleSavePipeline = (tasks: PipelineTask[]) => {
    const pipelineRequest: CreatePipelineRequest = {
      ...
      description: "Created via Web Console",
      steps: tasks.map(task => ({
        id: task.id, // Using the UUID generated by PipelineBuilder
        name: task.name,
        ...
        job_spec: {
          ...
          resources: { cpu_m: 100, memory_mb: 128, gpu: null },
          timeout_ms: task.timeout || 30000,
          retries: task.retryPolicy?.maxRetries || 3,
          env: {},
          secret_refs: []
        }
      }))
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-xs text-nebula-text-secondary">
                    <span>Última actualización: {new Date(pipeline.updated_at).toLocaleString()}</span>
                    <span>{pipeline.step_count} tareas</span>
                  </div>
                </div>

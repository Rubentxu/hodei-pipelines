...
pub trait WorkerProvider: Send + Sync + std::fmt::Debug {
    ...
    /// Create an ephemeral worker that will be automatically cleaned up after execution
    /// This is optimized for short-lived pipeline executions
...
pub struct ProviderConfig {
    ...
    /// Worker template defining resources, tools, volumes and environment
    pub template: WorkerTemplate,

    /// Custom Docker image to use (e.g., "hwp-agent:latest")
    /// Overrides the image specified in WorkerTemplate
    pub custom_image: Option<String>,

    /// Custom Kubernetes Pod manifest (YAML or JSON as String)
    /// Overrides the default pod spec generated from WorkerTemplate
    pub custom_pod_template: Option<String>,
}

impl ProviderConfig {
    pub fn docker(name: String, template: WorkerTemplate) -> Self {
        Self {
    ...
    pub fn kubernetes(name: String, template: WorkerTemplate) -> Self {
        Self {

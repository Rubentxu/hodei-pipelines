...
use hodei_pipelines_domain::WorkerStatus;
...
impl KubernetesProvider {
    ...
    fn create_pod_manifest(
        ...
        // Validate template
        if let Err(e) = config.template.validate() {
        ...
        if let Some(template) = &config.custom_pod_template {
            ...
            if let Ok(mut template_value) = serde_json::from_str::<Value>(template) {
                    ...
                    .and_then(|m| m.as_object_mut())
                {
                    ...
                    // Add template labels
                    for (key, value) in &config.template.labels {
                        ...
                        spec.get_mut("containers").and_then(|c| c.as_array_mut())
                {
                    for container in containers {
                        if let Some(env) = container.get_mut("env").and_then(|e| e.as_array_mut()) {
                            env.push(json!({"name": "WORKER_ID", "value": worker_id.to_string()}));
        ...
        let image = config
            .custom_image
            .as_deref()
            .unwrap_or(&config.template.container.image);

        // Build environment variables from template
        let mut env_vars = Vec::new();
        for env in &config.template.env {
            env_vars.push(json!({
                "name": env.name,
                "value": env.value
            }));
        ...
        // Build resource requirements from template
        let mut requests = json!({});
        let mut limits = json!({});

        if let Some(cpu) = &config.template.resources.cpu {
            requests["cpu"] = json!(cpu);
            limits["cpu"] = json!(cpu);
        }
        if let Some(memory) = &config.template.resources.memory {
            requests["memory"] = json!(memory);
            limits["memory"] = json!(memory);
        }
        if let Some(ephemeral_storage) = &config.template.resources.ephemeral_storage {
            requests["ephemeral-storage"] = json!(ephemeral_storage);
            limits["ephemeral-storage"] = json!(ephemeral_storage);
        }

        // Build volume mounts from template
        let mut volume_mounts = Vec::new();
        for vol in &config.template.volumes {
            let mut vm = json!({
                "name": vol.name,
                "mountPath": vol.mount_path
            });
            if let Some(sub_path) = &vol.sub_path {
                vm["subPath"] = json!(sub_path);
            }
            vm["readOnly"] = json!(vol.read_only);
            volume_mounts.push(vm);
        }
        ...
        // Build ports from template network config
        let mut container_ports = json!([]);
        if let Some(network) = &config.template.network {
            for port_map in &network.ports {
                container_ports.as_array_mut().unwrap().push(json!({
                    "containerPort": port_map.port,
...
impl WorkerProvider for KubernetesProvider {
    ...
    async fn create_ephemeral_worker(
        ...
        // Create manifest with ephemeral settings
        let mut manifest = KubernetesProvider::create_pod_manifest(&worker_id, &config, namespace)
            .map_err(|e| ProviderError::Provider(e.to_string()))?;
            ...
            if let Some(metadata) = manifest_obj.get_mut("metadata")
                && let Some(metadata_obj) = metadata.as_object_mut() {
                    ...
                    if let Some(labels) = metadata_obj.get_mut("labels") {
                        if let Some(labels_obj) = labels.as_object_mut() {
                            labels_obj.insert("hodei.worker.type".to_string(), "ephemeral".into());
                        }
                    } else {
                        metadata_obj.insert(
                            "labels".to_string(),
                            json!({
                                "hodei.worker.type": "ephemeral"
                            }),
                        );
                    }
                }
                        ...
                        if let Some(annotations) = metadata_obj.get_mut("annotations") {
                            if let Some(annotations_obj) = annotations.as_object_mut() {
                                annotations_obj.insert(
                                    "hodei.auto-cleanup-seconds".to_string(),
                                    cleanup_seconds.into(),
                                );
                            }
                        } else {
                            metadata_obj.insert(
                                ...
                                json!({
                                    "hodei.auto-cleanup-seconds": cleanup_seconds
                                }),
                            );
                        }
                    }
        ...
        if !response.status().is_success() {
            let error_text = response.text().await.unwrap_or_default();
            return Err(ProviderError::Provider(format!(
                "Failed to create ephemeral pod '{}': {}",
                pod_name, error_text
            )));
        }

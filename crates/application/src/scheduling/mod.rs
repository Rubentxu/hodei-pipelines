...
// GRC integration types
use hodei_pipelines_domain::resource_governance::{
    ComputePool, GlobalResourceController, PoolId, ResourceRequest,
};
use std::sync::Arc;
    ...
    WR: WorkerRepository + Send + Sync + 'static,
{
    ...
    pub rbac: Option<
        rbac::RoleBasedAccessControlService<
            R,
            hodei_pipelines_adapters::identity_access::rbac_repositories::InMemoryPermissionRepository,
            E,
        >,
    >, // Need generic P
    ...
    WR: WorkerRepository + Send + Sync + 'static,
{
    ...
    pub worker_management: Option<
        worker_management::WorkerManagementService<
            hodei_pipelines_adapters::scheduling::docker_provider::DockerProvider,
            SchedulerModule<R, E, W, WR>,
        >,
    >,
}
    ...
    WR: WorkerRepository + Send + Sync + 'static,
{
    ...
    pub fn build(self) -> Result<SchedulerModule<R, E, W, WR>> {
        let job_repo = self.job_repo.ok_or_else(|| {
            hodei_pipelines_domain::DomainError::Infrastructure("job_repository is required".into())
        })?;

        let event_bus = self.event_bus.ok_or_else(|| {
            hodei_pipelines_domain::DomainError::Infrastructure("event_bus is required".into())
        })?;

        let worker_client = self.worker_client.ok_or_else(|| {
            hodei_pipelines_domain::DomainError::Infrastructure("worker_client is required".into())
        })?;

        let worker_repo = self.worker_repo.ok_or_else(|| {
            hodei_pipelines_domain::DomainError::Infrastructure(
    ...
    WR: WorkerRepository + Send + Sync + 'static,
{
    ...
    async fn run_scheduling_cycle(&self) -> Result<()> {
        ...
        for entry in batch {
            ...
            // First, try to use GRC for pool selection if available
            if let Some(grc) = &self.global_resource_controller {
                // Find eligible pools using GRC
                let pools = self.find_eligible_pools(&job).await?;
            ...
            // Find eligible workers (traditional scheduling)
            ...
            if !workers.is_empty() {
                ...
                info!("No eligible workers found for job {}", job.id);
            }
        }
    ...
    pub fn job_to_resource_request(&self, job: &Job, tenant_id: Option<String>) -> ResourceRequest {
        use hodei_pipelines_domain::resource_governance::{RequestId, ResourceRequestBuilder};

        let mut required_labels = std::collections::HashMap::new();
        let mut preferred_labels = std::collections::HashMap::new();

        // Extract labels from job environment variables
        for (key, value) in &job.spec.env {
            if key.starts_with("label_") {
                let label_key = key.trim_start_matches("label_").to_string();
                preferred_labels.insert(label_key, value.clone());
            } else if key.starts_with("required_") {
                let label_key = key.trim_start_matches("required_").to_string();
                required_labels.insert(label_key, value.clone());
            }
        }

        let mut builder = ResourceRequestBuilder::new()
            .request_id(RequestId::from(job.id.to_string()))
        ...
        if let Some(gpu) = job.spec.resources.gpu {
            builder = builder.gpu_count(Some(gpu as u32));
        }
    ...
    pub async fn find_eligible_pools(&self, job: &Job) -> Result<Vec<ComputePool>> {
        if let Some(grc) = &self.global_resource_controller {
            ...
            grc.find_candidate_pools(&resource_request)
                .map_err(|e| DomainError::Infrastructure(format!("GRC error: {}", e)))
        } else {
            // GRC not configured, return empty vec
            Ok(vec![])
        }
    }
    ...
    pub async fn allocate_resources_for_job(&self, job: &Job, pool_id: PoolId) -> Result<()> {
        if let Some(grc) = &self.global_resource_controller {
            let resource_request = self.job_to_resource_request(job, None);
            let allocation_id = format!("alloc-{}", job.id);

            // Use GRC with quota check
            match grc.allocate_with_quota_check(allocation_id, pool_id.clone(), resource_request) {
                Ok(allocation) => {
                    info!(
                        "Allocated resources for job {} in pool {}: {:?}",
                        job.id, pool_id, allocation
                    );
                    Ok(())
                }
                Err(e) => Err(DomainError::Infrastructure(format!(
                    "GRC allocation failed: {}",
                    e
                ))),
            }
        } else {
            // GRC not configured, return ok
            Ok(())
        }
    }

use hodei_pipelines_adapters::security::JwtTokenService;
use hodei_pipelines_ports::security::TokenService;
use std::sync::Arc;
...
mod tests {
    use super::*;
    use hodei_pipelines_adapters::security::{JwtConfig, JwtTokenService};
    use hodei_pipelines_core::security::{Permission, Role};
    use hodei_pipelines_ports::security::SecurityError;
    use std::sync::Arc;
    ...
    impl TokenService for MockTokenService {
        ...
        ) -> hodei_pipelines_ports::security::Result<String> {
        ...
        fn verify_token(
            &self,
            token: &str,
        ) -> hodei_pipelines_ports::security::Result<hodei_pipelines_core::security::JwtClaims> {
            if self.should_fail || token != self.valid_token {
                return Err(SecurityError::Jwt("Invalid token".to_string()));
            }

            Ok(hodei_pipelines_core::security::JwtClaims {
        ...
        fn get_context(
            &self,
            _token: &str,
        ) -> hodei_pipelines_ports::security::Result<hodei_pipelines_core::security::SecurityContext> {
            Ok(hodei_pipelines_core::security::SecurityContext::new(
                "test-user".to_string(),
                vec![Role::Admin],

...
mod tests {
    ...
    async fn test_get_overall_status() {
        let sla_tracker = Arc::new(hodei_pipelines_modules::sla_tracking::SLATracker::new());
        let prioritization_engine =
            Arc::new(RwLock::new(QueuePrioritizationEngine::new(sla_tracker)));
        let wfq_config = hodei_pipelines_modules::weighted_fair_queuing::WFQConfig::default();

...
use hodei_pipelines_domain::{DomainError, JobId, Result, Worker, WorkerId};
...
use hodei_pipelines_ports::worker_template::WorkerTemplate;
    ...
    S: SchedulerPort + Send + Sync + Clone + 'static,
{
    ...
    pub async fn provision_worker(
        ...
        let config = ProviderConfig::docker(format!("worker-{}", worker_id), template);
    ...
    pub async fn get_worker_status(
        &self,
        worker_id: &WorkerId,
    ) -> Result<hodei_pipelines_domain::WorkerStatus> {
        let status = self
            .provider
...
impl From<DynamicPoolError> for hodei_pipelines_domain::DomainError {
    fn from(err: DynamicPoolError) -> Self {
        hodei_pipelines_domain::DomainError::Infrastructure(err.to_string())
    }
}

impl From<RemediationError> for hodei_pipelines_domain::DomainError {
    ...
    T: WorkerProvider + Send + Sync + Clone + 'static,
{
    ...
    async fn provision_workers_parallel(&self, count: u32, max_concurrent: u32) -> Result<()> {
        ...
        while provisioned < count {
            ...
            for _ in 0..to_spawn {
                ...
                let template = WorkerTemplate::new("worker", "latest", "ubuntu:22.04");
                let config = ProviderConfig::docker(
                    format!("{}-static-worker", self.config.pool_id),
                    template,
                );
    ...
    async fn provision_single_worker(&self, _index: u32) -> Result<()> {
        ...
        let template = WorkerTemplate::new("worker", "latest", "ubuntu:22.04");
        let config =
            ProviderConfig::docker(format!("{}-static-worker", self.config.pool_id), template);
    ...
    T: WorkerProvider + Send + Sync + Clone + 'static,
{
    ...
    async fn provision_worker(&self) -> Result<()> {
        let worker_id = WorkerId::new();
        let template = WorkerTemplate::new("worker", "latest", "ubuntu:22.04");
        let config = ProviderConfig::docker(format!("{}-worker", self.config.pool_id), template);
        let worker = self
            .worker_provider

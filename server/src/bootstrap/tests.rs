...
mod tests {
    use super::*;
    use hodei_adapters::config::{AppConfig, CacheConfig, DatabaseConfig, EventBusConfig};
    ...
    fn test_log_config_summary_does_not_panic() {
        let config = AppConfig {
            ...
            kubernetes: hodei_adapters::config::K8sGlobalConfig {
                insecure_skip_verify: false,
                ca_cert_path: None,
                token: None,
            },
            nats: hodei_adapters::config::NatsConfig {
                url: "nats://localhost:4222".to_string(),
                subject_prefix: "hodei.events".to_string(),
                connection_timeout_ms: 5000,
            },
            agent: hodei_adapters::config::AgentConfig {
                image: "hodei/hwp-agent:latest".to_string(),
                pull_policy: "IfNotPresent".to_string(),
            },
            server: hodei_adapters::config::ServerConfig {
                port: 8080,
                host: "0.0.0.0".to_string(),
            },
            logging: hodei_adapters::config::LoggingConfig {
                level: "info".to_string(),
                format: "json".to_string(),
            },
            tls: hodei_adapters::config::TlsConfig {
                enabled: false,
                cert_path: None,
                key_path: None,
            },

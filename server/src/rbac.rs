...
pub fn rbac_api_routes() -> Router<RbacApiAppState> {
    Router::new()
        ...
        .route("/auth/users", post(create_user_handler))
        .route("/auth/users/{id}", get(get_user_handler))
        .route("/auth/users/{id}", put(update_user_handler))
        .route("/auth/users/{id}", delete(delete_user_handler))
        .route("/auth/users/{id}/roles", get(get_user_roles_handler))
        .route("/auth/roles/assign", post(assign_role_handler))
        .route("/auth/roles/revoke", post(revoke_role_handler))
        .route("/auth/check", post(check_permission_handler))
}
